...
Hence for both of the above reasons we render the markdown to HTML ourselves, this gives us
complete control over formatting and syntax highlighting (where highlight.js does a better
job than bikeshed's Pygments) */
...
let argv = require('yargs')
    .boolean('bikeshed')
    .alias('b','bikeshed')
    .describe('bikeshed','Output in bikeshed format, default respec')
    .string('css')
    .alias('c','css')
    .describe('css','path to additional CSS file')
    .string('maintainers')
    .alias('m','maintainers')
...
function preface(title,options) {
    ...
    if (!options.bikeshed) {
        ...
        if (options.css) {
            preface += fs.readFileSync(options.css,'utf8').split('\n').join(' ');
        }
        preface += '</style>';
...
for (let l in lines) {
    ...
    if (!inCodeBlock && line.startsWith('#')) {
        ...
        if (!argv.bikeshed && (indent > 1)) {
            indent--;
        }
        let newIndent = indent;
        if (argv.bikeshed && (indent <= 2) && bsFix) {
            newIndent++;
        }
    ...
    if (!inCodeBlock && !argv.bikeshed && line.startsWith('#')) {
        let heading = 0;
